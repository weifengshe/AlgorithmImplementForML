 ...
 * @version 1.0
 */
public class TravelingSalesmanMIMICSample {
    /** The n value */
    ...
    public static void main(String[] args) throws Exception {
        ...
        int[] sample = new int[numIterations];
        sample[0] = 110;
        for (int i = 1; i < numIterations; i ++){
        	sample[i] = sample[i-1] + 10;
        ...
        for (int i=0; i < numIterations; i++) {
	        ...
	        MIMIC mimic = new MIMIC(spl, 100, pop);
...
//        System.out.println("MIMIC AVERAGE TRAINING TIME: " + avgGATrainingTimes );
...
//        pwtr.println("RHC SCORE, RHC TRAINING TIME, SA SCORE, SA TRAINING TIME, GA SCORE, GA TRAINING TIME, MIMIC SCORE, MIMIC TRAINING TIME");
        pwtr.println("SAMPLE, MIMIC SCORE, MIMIC TRAINING TIME");
        for(int i = 0; i < numIterations; i++) {
//            pwtr.println(RHCTrialBests[i] + "," + RHCTrainingTimes[i] + "," + SATrialBests[i] + "," + SATrainingTimes[i] + "," + GATrialBests[i] + "," + GATrainingTimes[i] + "," + mimicTrialBests[i] + "," + mimicTrainingTimes[i]);
        	  pwtr.println(sample[i] + "," + mimicTrialBests[i] + "," + mimicTrainingTimes[i]);
