package opt.test;
/** only change the temperature of Simulate Annealling algorithm;*/
import java.io.*;
 ...
 * @version 1.0
 */
public class KnapsackTestSATemperature {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int[] temperature = new int[numIterations];
        temperature[0] = 5;
        for (int i = 1; i < numIterations; i ++){
        	temperature[i] = temperature[i-1] + 1;
        ...
        for (int i=0; i < numIterations; i++) {
            ...
            SimulatedAnnealing sa = new SimulatedAnnealing(t, .95, hcp);
       ...
       File file = new File(outputDir + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss").format(new Date()) + "_Results.csv");
        ...
        pwtr.println("TEMPERATURE, SA SCORE, SA TRAINING TIME");
        ...
        for(int i = 0; i < numIterations; i++) {
        	...
        	pwtr.println(temperature[i] + "," + SATrialBests[i] + "," + SATrainingTimes[i]);
        }
        pwtr.println();
