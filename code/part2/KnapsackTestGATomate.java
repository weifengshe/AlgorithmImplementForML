package opt.test;
/** only change the tomate parameter of Genetic algorithm;*/
import java.io.*;
 ...
 * @version 1.0
 */
public class KnapsackTestGATomate {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int[] tomate = new int[numIterations];
        tomate[0] = 26;
        for (int i = 1; i < numIterations; i ++){
        	tomate[i] = tomate[i-1] + 3;
        ...
        for (int i=0; i < numIterations; i++) {
            ...
            StandardGeneticAlgorithm ga = new StandardGeneticAlgorithm(400, tm, 25, gap);
            ...
            System.out.println("POPULATION: " + tomate[i]);
       ...
       File file = new File(outputDir + new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss").format(new Date()) + "_Results.csv");
        ...
        pwtr.println("TOMATE, GA SCORE, GA TRAINING TIME");
        ...
        for(int i = 0; i < numIterations; i++) {
        	...
        	pwtr.println(tomate[i] + "," + GATrialBests[i] + "," + GATrainingTimes[i]);
        }
        pwtr.println();
